...
export async function POST(request: Request) {
  ...
  const purchased = user.purchasedPresets.some((id: unknown) => String(id) === presetId);
  if (!purchased) return NextResponse.json({ success: false, error: "Not purchased" }, { status: 403 });

  await connectToDatabase();
  const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
  await downloadTokensController.create({ presetId, userId: user._id, expiresAt: expiresAt.toISOString() });
